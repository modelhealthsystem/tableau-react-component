...
export default (props => {
  ...
  const getURL = url => {
    ...
    if (url && typeof url === 'string' && url.trim() && url.match(regex)) return url;else error = {
      flag: true,
  ...
  const getFilters = filters => {
    if (filters && (typeof filters !== 'object' || Array.isArray(filters))) {
      ...
      return {};
    } else return Object.assign({}, filters);
  };

  const getParameters = parameters => {
    if (parameters && (typeof parameters !== 'object' || Array.isArray(parameters))) error = {
      flag: true,
      message: 'Incorrect Parameter type'
    };else return Object.assign({}, parameters);
  };

  const getOptions = options => {
    if (options && (typeof options !== 'object' || Array.isArray(options))) error = {
      flag: true,
  ...
  const getQuery = query => {
    if (query && (typeof query !== 'object' || Array.isArray(query))) error = {
      flag: true,
      message: 'Incorrect Query type'
    };else {
      const standardisedQuery = Object.assign({}, query);
      return Object.keys(standardisedQuery).map((key, idx) => {
        return `${idx === 0 ? '?' : '&'}:${key}=${standardisedQuery[key]}`;
      }).join('');
    }
  };

  const getFilterURL = (url, filters) => {
    const filterString = Object.keys(filters).map((key, idx) => {
  ...
  }, "View on Tableau Server") : React.createElement("div", {
